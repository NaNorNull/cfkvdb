class Db {
    ...
    async setToken(aToken) {
        // Use the token to assign the database area (not shared publicaly)
        ...
        try {
            ...
            if (jsonString === null || jsonString === "") {
                ...
                return { "error": "ST002: Unathorized - token value missing " + aToken };
            } else {
    ...
    async listTypes() {
        ...
        if (this.area === null) {
            log("ERROR", "LS001: Unathorized - token unassigned");
            return { "types": null, "error": "LS001: Unathorized - token unassigned" }
        }
...
function getKeyPrefix(area, type) {
    ...
    if (area === null) {
        log("ERROR", "GKP001: Unathorized - token unassigned");
        return { "keyPrefix": null, "error": "GKP001: Unathorized - token unassigned" };
    }

...
//    This can be used to create seperate areas by user, etc.
//    token must be written out-of-band with a key of "db_meta_token_" + token
//    It must be of the format '{ "area": "user1" }' as limited above. 
//    See getToken in cf-kv-db.ks.
//    It is assumed this would handled as part of a "login" mechanism
...
//              Runs a series of tests and displays the results.  This is only avialable if 
...
//    for 60 seconds.
//
// Intential Exclusions: Sorting, REST, POST
//
...
async function handleRequest(request) {
  ...
  // for testing. getToken is not visibile externally in this service and 
  // should not be.
  // See wrangler.toml to assign ALLOW_TEST to true

  if (urlPath === "/" + urlPrefix + "/1/test" && typeof ALLOW_TEST == 'string' && ALLOW_TEST == "True") {
    const anArea = "testArea" + crypto.randomUUID().replace(/-/g, "");

    const { token, error } = await db.getToken({ "area": anArea }, 60);
    if (error === null) {
      return Response.redirect(request.url.replace("/1/test", "/" + token.id + "/test"));
    } else {
  ...
  if (route === "test" && typeof ALLOW_TEST == 'string' && ALLOW_TEST == "True") {
